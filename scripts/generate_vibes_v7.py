#!/usr/bin/env python3
"""
generate_vibes_v7.py — Trial 07: the 808 frame + Trell's evolved riding-trap bass rules.
...
def score_pack(bass, melody, kick, target_key):
    ...
    if bass:
        ...
        if bar1 and sum(1 for p in bar1 if p <= BAR1_CEIL) / len(bar1) > 0.5:
            s += 1                                              # bar-1 ceiling
        if all(any(p < root + 7 for _, p, _ in bar_slice(bass, b) or [(0, 0, 0)])
               for b in range(N_BARS)):
            s += 1                                              # no retired structure
...
def main():
    ...
    combos = [
        {"id": "v01", "weights": T6_V03_W, "label": "t6v03weights-808check"},
        {"id": "v02", "weights": T6_V07_W, "label": "t6v07weights-808check"},
    ]
    for i in range(6):
    ...
    for idx, combo in enumerate(combos):
        ...
        candidates = [build_candidate(packs, lib, cfg, weights,
                                      9900 + idx * 37 + k, 10000 + idx * 37 + k)
                      for k in range(N_CANDIDATES)]
        best = max(candidates, key=lambda c: c["score"])

        folder = OUT_DIR / f"t7 {combo['id']} {combo['label']}"
        ...
        for role, (notes, p) in best["drums_extra"].items():
            ...
            drum_notes += [(t, DRUM_PITCH[role], min(d, 200), velocity(role, t % BAR, cfg, rng))
                           for (t, _, d) in notes]
        write_midi(folder / f"t7 {combo['id']} - drums.mid", [("drums", 9, drum_notes)], bpm)
        write_midi(folder / f"t7 {combo['id']} - bassline.mid",
                   [("bassline", 0, [(t, p, d, velocity("bass", t % BAR, cfg, rng))
                                     for (t, p, d) in best["bass"]])], bpm)
        write_midi(folder / f"t7 {combo['id']} - melody.mid",
        ...
        manifest.append({"id": combo["id"], "label": combo["label"], "trial": 7,
        ...
        print(f"t7 {combo['id']}  {combo['label']:<22} key {best['key']:<4} score {best['score']}"
              f"  bar1: {' '.join(manifest[-1]['bass_bar1_notes']):<24}"
              f" lands on {manifest[-1]['bass_last_note']}")

    (OUT_DIR / "manifest.json").write_text(json.dumps(
        {"generated": str(date.today()), "trial": 7, "combos": manifest}, indent=1))
    write_manifest_csv(OUT_DIR / "manifest.csv", "t7", manifest)
